...
func (l *Logger) LogRequest(r *http.Request, body []byte) {
	...
	if l != nil && l.logFn != nil {
		...
		msg := fmt.Sprintf("Sending request to %s", r.URL.Hostname())
		l.logFn(r.Context(), msg,
			"url", r.URL.Redacted(),
			"method", r.Method,
			"headers", headers,
...
func (l *Logger) LogResponse(r *http.Response, body []byte) {
	if l != nil && l.logFn != nil {
		msg := fmt.Sprintf("Received response from %s", r.Request.URL.Hostname())
		l.logFn(r.Request.Context(), msg,
